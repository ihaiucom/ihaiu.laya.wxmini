...
interface _getFileInfoObject {
   ...
   * 接口调用失败的回调函数
   */
  fail?: (result: _getFileInfoSuccessObject) => void;

  /**
   * 接口调用结束的回调函数（调用成功、失败都会执行）
   */
  complete?: (result: _getFileInfoSuccessObject) => void;
}
...
declare namespace wx {
   ...
   */
  export function downloadFile(object: _downloadFileObject): void | DownloadTask;
